...
exports.Bundle = class {
    ...
    static create(bundler, config) {
        ...
        return Utils.runSequentially(dependenciesToBuild, dep => bundler.configureDependency(dep))
            .then(descriptions => {
            return Utils.runSequentially(descriptions, description => {
    ...
    getBundledFiles() {
        ...
        let bundleFiles = uniqueBy(this.includes.reduce((a, b) => a.concat(b.getAllFiles()), []), file => file.path).sort((a, b) => {
            // alphabetical sorting based on moduleId
            if (a.moduleId > b.moduleId)
                return 1;
            if (b.moduleId > a.moduleId)
                return -1;
            return 0;
        });
        const sorted = [];
            ...
            if (dependencyInclusion) {
                ...
                if (deps) {
                    deps.forEach(packageName => {
                        bundleFiles.filter(f => f.dependencyInclusion && f.dependencyInclusion.description.name === packageName).forEach(visit);
                    });
                }
            }
            sorted.push(file);
        ...
        while (true) { // eslint-disable-line no-constant-condition
            const idx = sorted.findIndex(f => f.dependencyInclusion && (f.dependencyInclusion.description.name === 'jquery' ||
    ...
    write(platform) {
        ...
        return work.then(() => {
            ...
            for (let i = 0; i < files.length; ++i) {
                ...
                concat.add(currentFile.path ? path.relative(process.cwd(), currentFile.path) : null, content, sourceMap && sourceMap.sources.length ? JSON.stringify(sourceMap) : undefined);
            }
            let bundleMap;
            ...
            let mapSourceRoot = path.posix.relative(path.posix.join(process.cwd(), platform.output), process.cwd());
    ...
    writeBundlePathsToIndex(platform) {
            ...
            try {
                ...
                if (configMatcher.test(indexFile)) {
                    ...
                    if (newIndexFile !== indexFile) {
                        yield fs.writeFile(platform.index, newIndexFile);
